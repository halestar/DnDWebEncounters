...
<form action="{{ route('play.finish.monster_turn', ['adventure_encounter' => $adventureEncounter->id, 'adventure_actor' => $currentActor->id]) }}" method="POST" id="monster_turn_form">
    ...
    <div class="border border-info rounded p-2 monster-health-widget mb-3">
        ...
        <div class="row">
            <div class="col-3 text-center">
                <input type="text" class="border border-dark rounded text-center" name="current_hp" id="current_hp"
                       value="{{ $currentActor->current_hp }}" style="width: 64px; height:64px; font-size: 32px"/>
            </div>
            <div class="col-9">
                <div class="w-100 d-flex align-content-stretch flex-nowrap mb-2">
                    <button type="button" class="flex-grow-1 btn btn-secondary m-1" style="width: 44px; height: 44px;"
                            onclick="update_monster_hp(jQuery('input[name=dmg_type]:checked').val(), 1, {{ $currentActor->max_hp }})">
                        1
                    </button>
                    <button type="button" class="flex-grow-1 btn btn-secondary m-1" style="width: 44px; height: 44px;"
                            onclick="update_monster_hp(jQuery('input[name=dmg_type]:checked').val(), 2, {{ $currentActor->max_hp }})">
                        2
                    </button>
                    <button type="button" class="flex-grow-1 btn btn-secondary m-1" style="width: 44px; height: 44px;"
                            onclick="update_monster_hp(jQuery('input[name=dmg_type]:checked').val(), 3, {{ $currentActor->max_hp }})">
                        3
                    </button>
                    <button type="button" class="flex-grow-1 btn btn-secondary m-1" style="width: 44px; height: 44px;"
                            onclick="update_monster_hp(jQuery('input[name=dmg_type]:checked').val(), 4, {{ $currentActor->max_hp }})">
                        4
                    </button>
                    <button type="button" class="flex-grow-1 btn btn-secondary m-1" style="width: 44px; height: 44px;"
                            onclick="update_monster_hp(jQuery('input[name=dmg_type]:checked').val(), 5, {{ $currentActor->max_hp }})">
                        5
                    </button>
                </div>
                <div class="d-flex align-content-stretch flex-nowrap mb-2">
                    <button type="button" class="flex-grow-1 btn btn-secondary m-1" style="width: 44px; height: 44px;"
                            onclick="update_monster_hp(jQuery('input[name=dmg_type]:checked').val(), 6, {{ $currentActor->max_hp }})">
                        6
                    </button>
                    <button type="button" class="flex-grow-1 btn btn-secondary m-1" style="width: 44px; height: 44px;"
                            onclick="update_monster_hp(jQuery('input[name=dmg_type]:checked').val(), 7, {{ $currentActor->max_hp }})">
                        7
                    </button>
                    <button type="button" class="flex-grow-1 btn btn-secondary m-1" style="width: 44px; height: 44px;"
                            onclick="update_monster_hp(jQuery('input[name=dmg_type]:checked').val(), 8, {{ $currentActor->max_hp }})">
                        8
                    </button>
                    <button type="button" class="flex-grow-1 btn btn-secondary m-1" style="width: 44px; height: 44px;"
                            onclick="update_monster_hp(jQuery('input[name=dmg_type]:checked').val(), 9, {{ $currentActor->max_hp }})">
                        9
                    </button>
                    <button type="button" class="flex-grow-1 btn btn-secondary m-1" style="width: 44px; height: 44px;"
                            onclick="update_monster_hp(jQuery('input[name=dmg_type]:checked').val(), 10, {{ $currentActor->max_hp }})">
                        10
                    </button>
                </div>
            </div>
